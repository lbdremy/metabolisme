"""`autoroutes reproduce` — replay every stabilized pipeline stage from raw data.

Each stabilized stage is listed here and re-run in order, so
`uv run autoroutes reproduce` always rebuilds every published result from the
...
STAGES: tuple[tuple[str, Callable[[Path], int]], ...] = (("rente-d15", build.run_rent),)
